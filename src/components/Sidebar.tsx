...
type SidebarProps = {
  ...
  activeView: 'editor' | 'chat' | 'slides'
  onViewChange: (view: 'editor' | 'chat' | 'slides') => void
  chats: ChatSession[]
...
export function Sidebar({ workspace, activeView, onViewChange, chats, selectedChatId, onNewChat, onSelectChat, onDeleteChat, selectedPath, onSelectNote, onChangeWorkspace, onWorkspaceChanged, onGenerateNote, onDelete }: SidebarProps) {
  ...
  return <aside className="flex w-[280px] shrink-0 flex-col border-r border-zinc-800/80 bg-zinc-950">
    ...
    <div className="grid grid-cols-3 border-b border-zinc-800/80 p-2"><button type="button" onClick={() => onViewChange('editor')} className={`py-2 text-xs transition ${activeView === 'editor' ? 'bg-zinc-800 text-emerald-300' : 'text-zinc-600 hover:text-zinc-300'}`}>Notlar</button><button type="button" onClick={() => onViewChange('chat')} className={`py-2 text-xs transition ${activeView === 'chat' ? 'bg-zinc-800 text-emerald-300' : 'text-zinc-600 hover:text-zinc-300'}`}>Sohbetler</button><button type="button" onClick={() => onViewChange('slides')} className={`py-2 text-xs transition ${activeView === 'slides' ? 'bg-zinc-800 text-emerald-300' : 'text-zinc-600 hover:text-zinc-300'}`}>Slayt</button></div>
    {activeView === 'chat' ? <ChatSidebar chats={chats} selectedChatId={selectedChatId} onNewChat={onNewChat} onSelectChat={onSelectChat} onDeleteChat={onDeleteChat} /> : <><div className="flex items-center justify-between px-5 pb-2 pt-6"><span className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-600">Workspace</span><div className="flex items-center gap-1"><button type="button" onClick={() => void createFolder(workspace.path)} title="Yeni klasör" className="rounded p-1 text-zinc-600 transition hover:bg-zinc-800 hover:text-emerald-300"><FolderPlus size={15} /></button><button type="button" onClick={() => void createMarkdownNote(workspace.path)} title="Yeni not" className="rounded p-1 text-zinc-600 transition hover:bg-zinc-800 hover:text-emerald-300"><FilePlus size={15} /></button><button type="button" onClick={onChangeWorkspace} title="Workspace değiştir" className="rounded p-1 text-zinc-600 transition hover:bg-zinc-800 hover:text-emerald-300"><HardDrive size={15} /></button></div></div><div className="relative flex-1 overflow-y-auto px-3 pb-5"><EntryTree entry={workspace} depth={0} selectedPath={selectedPath} onSelectNote={onSelectNote} onCreateDirectory={createFolder} onCreateNote={createMarkdownNote} onGenerateNote={onGenerateNote} onDelete={onDelete} onContextMenu={openContextMenu} />{error && <p className="mt-4 px-2 text-xs leading-5 text-red-300">{error}</p>}{contextMenu && <div onClick={(event) => event.stopPropagation()} style={{ left: contextMenu.x, top: contextMenu.y }} className="fixed z-50 min-w-36 border border-zinc-700 bg-zinc-900 py-1 shadow-xl shadow-black/40"><button type="button" onClick={() => { setContextMenu(undefined); void onDelete(contextMenu.entry) }} className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs text-red-300 transition hover:bg-red-400/10"><Trash2 size={14} />Sil</button></div>}</div></>}

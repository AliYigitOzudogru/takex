...
export function Editor({ path, name, content, onSaved }: EditorProps) {
  ...
  return <section className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-zinc-950">
    ...
    {isEditing ? <textarea ref={editorRef} value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => void handleKeyDown(event)} onBlur={() => void persist()} className="min-h-0 flex-1 resize-none overflow-y-auto bg-zinc-950 px-8 py-8 font-mono text-sm leading-7 text-zinc-300 outline-none md:px-16" spellCheck={false} aria-label="Markdown düzenleyici" /> : <div onDoubleClick={() => setIsEditing(true)} className="min-h-0 flex-1 cursor-text overflow-y-auto overscroll-contain"><article className="markdown-content mx-auto max-w-3xl px-8 pb-24 pt-12 md:px-16"><ReactMarkdown rehypePlugins={[rehypeHighlight]}>{content}</ReactMarkdown></article></div>}
  </section>
}
